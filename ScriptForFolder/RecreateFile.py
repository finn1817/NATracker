...
def main():
    fileToGrab = "834968.journal"
    journal = getJournal(fileToGrab)
    print (recreateFile(journal))
    print ("First Revision")
    print (recreateUpToEntry(journal, journal.JournalEntrys[0]))
    print ("2nd Revision")
    for entry in journal.JournalEntrys:
        for change in entry:
            change_time = datetime.datetime.fromtimestamp(journal.JournalEntrys[0][0][3]
).strftime('%Y-%m-%d %H:%M:%S')
            print(f"Change at {change_time}: {change}")
    print (recreateUpToEntry(journal, journal.JournalEntrys[1]))
    print ("Third Revision")
    print (recreateUpToEntry(journal, journal.JournalEntrys[2]))
    print ("Fourth Revision")
    print (recreateUpToEntry(journal, journal.JournalEntrys[3]))
